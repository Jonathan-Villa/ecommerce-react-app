...
const stepStateCount = (state, action) => {
  switch (action.type) {
    case "INCREMENT": return  {activeStep: action.count + 1} 
    case "DECREMENT": return  {activeStep: action.count - 1 }
    case "RESET": return  {activeStep: action.count }
...
const stepContent = (index, cart, store) => {
  switch (index) {
    case 0: return <ShoppingCart cart={cart} store={store} />;
    case 1: return <CheckOutOrder cart={cart} store={store} />;
    ...
    <Grid container={true} className={classes.root}>
      ...
      >
        ...
        {state.activeStep === getSteps.length ? (
          ...
          <Grid  className={classes.displayItemsWrapper}>
